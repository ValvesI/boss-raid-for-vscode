	...
	public start(onProgress: (progress: CodeProgress) => void): vscode.Disposable {
		...
		const changeDocumentDisposable = vscode.workspace.onDidChangeTextDocument((event) => {
			...
			const progress = event.contentChanges.reduce<CodeProgress>(
				(total, change) => ({
					charactersAdded:
						total.charactersAdded + countAddedCharacters(change.text),
					linesAdded: total.linesAdded,
					linesRemoved: total.linesRemoved,
				}),
				{ charactersAdded: 0, linesAdded: 0, linesRemoved: 0 },
			);

			const lineDifference = currentNonEmptyLines - previousNonEmptyLines;
			progress.linesAdded = Math.max(0, lineDifference);
			progress.linesRemoved = Math.max(0, -lineDifference);
